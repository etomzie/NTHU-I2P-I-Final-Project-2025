...
class BattleScene(Scene):
    def __init__(self):
        ...
        monster_data = self.game_manager.bag._monsters_data
        dist = 10 + 50  
        for i, now in enumerate(monster_data):
    ...
    def draw(self, screen: pg.Surface) -> None:
        ...
        if self.fight_started:
            monster_data = self.game_manager.bag._monsters_data
            current_monster = monster_data[self.current_monster_index]
            #print(my_monsters)
            pg.font.init()
            ...
            start_y = 25
            self.draw_monster(screen ,start_x, start_y, monster_data[1])
            ...
            monster = Sprite(monster_data[1]["sprite_path"])
            monster.image = pg.transform.scale_by(monster.image, 8)
            screen.blit(monster.image, (GameSettings.SCREEN_WIDTH / 5 * 3, GameSettings.SCREEN_HEIGHT / 3 - 100))
